      ...
      session_start();
				...
				echo'
					<tr align="center">
						...
						if(isset($_SESSION["user"])){
							echo '
								<td><input class="btn btn-secondary" type="button" value="Ver" onclick="verImagen('.$i.')"><input type="hidden" id="'.$i.'" value="'.$pagos['imagenFolio'].'"></td>
								<td>'.$pagos['folio'].'</td>
		...
		public static function addPaymentController(){
			if(isset($_POST["aceptar"])){
				...
				$data = array("grupo" => (int)$_POST["grupo"], "alumna"=>(int)$_POST["alumna"], "nombreMama"=>$_POST["nombreMama"], "apellidoMama"=>$_POST["apellidoMama"], "fechaPago" => $_POST["fechaPago"], "fechaEnvio" =>date("Y-m-d H:i:s"), "imagenFolio" =>$dirFile, "folio" =>$_POST["folio"]);
		...
		public static function loginController(){
			if(isset($_POST["login"])){//Se condiciona si se utilizo el boton de nombre "login"
				...
				if($answer["usuario"] == $_POST["usuario"] && $answer["password"] == $_POST["password"]){
					//session_start();
					$_SESSION['user'] = true;//A la variable de sesion se le asigna el valor del id del usuario que ingreso
		...
		//Metodo que mediante el uso del modelo editStudentModel, la extraccion de los datos de la base de datos dependiendo del id de la alumna que se tenga y se imprimiran dichos datos para poder visualizarlos y poder editarlos
		...
		//Este metodo utilizara los datos que se encuentren dentro del metodo editStudentController, ya que son lo que se utilizaran para hacer la modificacion de la alumna
		...
		############################################################################################################################################################3
		###########3##################################################### LUGARES CONTROLLER !$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$#
		#############################################################################################################################################################

		//Metodo que mediante el uso del modelo editPaymentModel, la extraccion de los datos de la base de datos dependiendo del id del pago que se tenga y se imprimiran dichos datos para poder visualizarlos y poder editarlos
		...
		//Este metodo utilizara los datos que se encuentren dentro del metodo editGroupController, ya que son lo que se utilizaran para hacer la modificacion del pago
		...
		//Metodo del controlador para tomar el id del pago y mandarlo al modelo para realizar la eliminacion en la base de datos
		...
		public function studentGroupController(){
		...
		$answer = Data::showStudentsModel("alumnas","grupo");
			...
			$showStudent ="";
			foreach ($answer as $student) {
				$showStudent = $showStudent . $student["id_alumna"] . "," . $student["nombre"] . " " . $student["apellidos"] .','. $student["grupo"] . "%";
			}
			return $showStudent;
	  }	
	}
