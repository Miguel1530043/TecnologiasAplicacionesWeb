...
<?php
	class MvcController{
		...
		public static function loginController(){
			if(isset($_POST["login"])){//Se condiciona si se utilizo el boton de nombre "login"
				...
				if($answer["username"] == $_POST["username"] && $answer["password"] == $_POST["password"]){
					...
					echo"<script>
						window.location='index.php?action=dashboard&id_store=$answer[id_store]'
					</script>";
		...
		public static function storeViewsController(){
			$answer = Data::storeViewsModel("store");
			$estado="";
			foreach ($answer as $row => $store) {//Ciclo para imprimir cada uno de los registros de la tabla
				...
				<tr>
					...
					<td>'.$estado.'</td>
					<td><a href="index.php?action=editStore&id='.$store['store_id'].'" class="btn btn-primary">Editar</a></td>
					<td><a href="index.php?action=deleteStore&id='.$store['store_id'].'" class="btn btn-danger">Eliminar</a></td>
					<td><a href="index.php?action=viewStore&id_store='.$store['store_id'].'" class="btn btn-info">Ver</a></td>
				</tr>';
			}
		...
		public static function editStoreController(){
			$id = $_GET['id'];//Se recibe el id del producto el cual es mandado por variable de url
			$answer = Data::editStoreModel($id,"store");//Se decalara una varibale que tendra el resultado del uso del modelo de editar producto
			...
			echo '
				<input type="hidden" value="'.$answer['store_id'].'" name="store_id" class="form-control">

				<div class="col-2"></div>
				<div class="col-8">
					<label>Nombre de la Tienda</label><input type="text" value="'.$answer['store_name'].'" name="store_name" class="form-control"><br>
				</div>
				<div class="col-2"></div>

				<div class="col-2"></div>
				<div class="col-4">
					<label>Descripcion</label><input type="text" value="'.$answer['store_description'].'" name="store_description" class="form-control">
				</div>
				<div class="col-4">
					<label>Direccion</label><input type="text" value="'.$answer['store_address'].'" name="store_address" class="form-control"><br>
				</div>
				<div class="col-2"></div>
		...
		public static function updateStoreController(){
			...
			if(isset($_POST['update'])){//Se condiciona se hizo uso del boton de nombre "update" 
				...
				$data= array("store_id"=>$_POST['store_id'],"store_name"=>$_POST["store_name"],"store_description"=>$_POST["store_description"],"store_address"=>$_POST["store_address"],"status"=>$_POST["status"]);
				...
				$answer = Data::updateStoreModel($data,"store");
		...
		public static function storeInfoController(){
			$id_store = $_GET['id_store'];
			$answer = Data::storeInfoModel($id_store,"store");
			echo $answer["store_name"];
		}
		...
		public static function productViewsController(){
			$id_store=$_GET['id_store'];
			$answer = Data::productStoreViewsModel("products","category",$id_store);
			...
			foreach ($answer as $row => $product) {//Ciclo para imprimir cada uno de los registros de la tabla
				...
				<tr>
					...
					<td><a href="index.php?action=editProduct&id='.$product['id_prod'].'&id_store='.$id_store.'" class="btn btn-primary">Editar</a></td>
					<td><a href="index.php?action=deleteProduct&id='.$product['id_prod'].'&id_store='.$id_store.'" class="btn btn-danger">Eliminar</a></td>
					<td><a href="index.php?action=viewProduct&id='.$product['id_prod'].'&id_store='.$id_store.'" class="btn btn-info">Operaciones</a></td>
				</tr>';
			}
		...
		public static function addProductController(){
			if(isset($_POST["add"])){
				...
				//Se declara un arreglo asociativo con los valores de los campos del formulario de registro de producto
				$data = array("code"=>$_POST['code'],"prod_name"=>$_POST["prod_name"],"date_added"=>date("Y-m-d"),"price"=>$_POST["price"],"stock"=>$_POST["stock"],"category_id"=>$_POST["category_id"],"id_store"=>$_GET["id_store"]);
				...
				if($answer=="success"){//Se condiciona si el Modelo de registro devuelve "success" se redireccione al apartado de productos
					echo "<script>
							window.location='index.php?action=products&id_store=$data[id_store]'
		...
		public static function updateProductController(){
			...
			if(isset($_POST['update'])){//Se condiciona se hizo uso del boton de nombre "update" 
				...
				if($answer=="success"){//Si el modelo de actualizar Producto devuelve "success" se redirecciona al apartado de productos
						echo "<script>
							window.location='index.php?action=products&id_store=".$id_store."
		...
		public static function userViewController(){
			...
			$answer = Data::userStoreViewModel("user");
		...
		/*public static function countUserStoreController(){
			$id_store = $_GET["id"];
			$action Data::userViewModel()
		}*/
		...
		public static function addUserController(){
			...
			if(isset($_POST["add"])){//Se condiciona si se hizo uso del botton "add"
				...
				$data = array("firstname"=>$_POST['firstname'],"lastname"=>$_POST["lastname"],"username"=>$_POST['username'],"password"=>$_POST["password"],"email"=>$_POST["email"],"date_added"=>date("Y-m-d"),"id_store"=>$_GET["id_store"]);
				...
				if($answer=="success"){//Si el modelo retorna "success" se redireccionara al apartado de usuarios
					echo "<script>
						window.location='index.php?action=users&id_store=$data[id_store]'
					</script>";
		...
		public static function categoryViewController(){
			...
			$id_store = $_GET["id_store"];
			$answer = Data::categoryStoreViewModel("category",$id_store);
		...
		public static function deleteCategoryController(){
			if(isset($_POST['delete'])){//Se condiciona si se hizo uso del boton "delete"
				...
				if($answer=="success"){//Si el modelo retorna success se redireccionara a las categorias
						echo "<script>
							window.location='index.php?action=categories&id_store=".$id_store."
						</script>";
		...
		public static function countHistorialController(){
			$answer = Data::viewStoreHistorialModel("historial");
			echo count($answer);		
		}
